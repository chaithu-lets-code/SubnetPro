...
/* ╔══════════════════════════════════════════════════════════════════╗
   ║  OSPF SIMULATOR · Phase 6 : Area Type Comparator                ║
   ║  Standard · Stub · Totally Stub · NSSA · Totally NSSA           ║
   ╚══════════════════════════════════════════════════════════════════╝ */

/* ═══════════════════════════════════════════════════════
   PHASE 6 STATE
   ═══════════════════════════════════════════════════════ */
let atcCanvas, atcCtx;
let atcAreaType   = 'standard';  // active area type
let atcAnimId     = null;
let atcPulseT     = 0;
let atcPulseNode  = null;
let atcPulseColor = '#22c55e';

/* ── LSA type metadata ── */
const ATC_LSA_TYPES = [
  { key:'T1', label:'Type 1',  name:'Router LSA',      color:'#3b82f6' },
  { key:'T2', label:'Type 2',  name:'Network LSA',     color:'#22c55e' },
  { key:'T3', label:'Type 3',  name:'Summary LSA',     color:'#f59e0b' },
  { key:'T4', label:'Type 4',  name:'ASBR Summary',    color:'#8b5cf6' },
  { key:'T5', label:'Type 5',  name:'AS External',     color:'#ef4444' },
  { key:'T7', label:'Type 7',  name:'NSSA External',   color:'#06b6d4' },
  { key:'T0', label:'Default', name:'Default Route',   color:'#a78bfa' },
];

/* ── Area type definitions ── */
const ATC_AREA_TYPES = {
  standard: {
    label:   'Standard Area',
    color:   '#3b82f6',
    cmd:     '! (no special config — default)',
    allowed: ['T1','T2','T3','T4','T5'],
    blocked: ['T7'],
    hasDefault: false,
    hasT7:   false,
    lsdbInStub:  ['T1','T2','T3','T4','T5'],
    rtEntries: [
      { prefix:'10.0.0.0/24',  type:'O',    cost:1,  via:'R1(ABR)' },
      { prefix:'10.1.0.0/24',  type:'O IA', cost:2,  via:'R1(ABR)' },
      { prefix:'172.16.0.0/16',type:'O IA', cost:3,  via:'R1(ABR)' },
      { prefix:'203.0.113.0/24',type:'O E2',cost:20, via:'R1(ABR)' },
    ],
    desc: 'Standard area — no restrictions. All LSA types (T1–T5) flood freely. The stub area router maintains a complete LSDB including external routes (Type 5). Routing table contains full topology and all external routes.\n\nUse when: routers have sufficient memory/CPU, full external route visibility is needed.',
    rfc: 'RFC 2328 §1.3 — Standard areas have no LSA restrictions',
  },
  stub: {
    label:   'Stub Area',
    color:   '#22c55e',
    cmd:     'router ospf 1\n area 1 stub\n! (configure on ALL routers in area)',
    allowed: ['T1','T2','T3'],
    blocked: ['T4','T5','T7'],
    hasDefault: true,
    hasT7:   false,
    lsdbInStub:  ['T1','T2','T3','T0'],
    rtEntries: [
      { prefix:'10.0.0.0/24',   type:'O',    cost:1, via:'Direct'   },
      { prefix:'10.1.0.0/24',   type:'O IA', cost:2, via:'R1(ABR)'  },
      { prefix:'172.16.0.0/16', type:'O IA', cost:3, via:'R1(ABR)'  },
      { prefix:'0.0.0.0/0',     type:'O*IA', cost:1, via:'R1(ABR)'  },
    ],
    desc: 'Stub area — blocks Type 4 and Type 5 LSAs. The ABR (R1) injects a default route (Type 3) into the stub area. Internal routers use this default for all external destinations.\n\nReduces LSDB size significantly — external routes replaced by a single default route.\n\nConstraints: Cannot contain an ASBR. Cannot be the backbone (Area 0).',
    rfc: 'RFC 2328 §3.6 — Stub areas: no Type 4/5 LSAs, default route injected',
  },
  totallystub: {
    label:   'Totally Stub',
    color:   '#f59e0b',
    cmd:     'router ospf 1\n area 1 stub no-summary\n! (only on ABR — internal routers use "area 1 stub")',
    allowed: ['T1','T2'],
    blocked: ['T3','T4','T5','T7'],
    hasDefault: true,
    hasT7:   false,
    ...
    desc: 'Totally Stub area (Cisco extension) — blocks Type 3, 4, and 5 LSAs. Only a single Type 3 default route enters from the ABR. Internal routers have the smallest possible LSDB.\n\nMinimal memory and CPU. Internal routers only know their own area topology + one default route for everything else.\n\nConstraints: Cannot contain ASBR. Only ABR needs "no-summary" — all other routers use "area X stub".',
    rfc: 'RFC 2328 §3.6 + Cisco extension — no-summary blocks all Type 3s except default',
  },
  nssa: {
    label:   'NSSA',
    color:   '#06b6d4',
    cmd:     'router ospf 1\n area 1 nssa\n! (all routers in area must have nssa configured)',
    allowed: ['T1','T2','T3','T7'],
    blocked: ['T4','T5'],
    hasDefault: false,
    hasT7:   true,
    lsdbInStub:  ['T1','T2','T3','T7'],
    rtEntries: [
      { prefix:'10.0.0.0/24',   type:'O',    cost:1,  via:'Direct'   },
      { prefix:'10.1.0.0/24',   type:'O IA', cost:2,  via:'R1(ABR)'  },
      { prefix:'172.16.0.0/16', type:'O IA', cost:3,  via:'R1(ABR)'  },
      { prefix:'192.0.2.0/24',  type:'O N2', cost:20, via:'R6(ASBR)' },
    ],
    desc: 'NSSA (Not-So-Stubby Area) — blocks Type 4 and Type 5 LSAs but allows a local ASBR to redistribute external routes using Type 7 LSAs.\n\nThe ABR (R1) translates Type 7 → Type 5 and floods into Area 0.\n\nKey advantage: allows external redistribution in a stub-like area. Type 7 stays within the NSSA. The ABR with highest RID performs translation.\n\nDefault route NOT injected automatically — needs: area X nssa default-information-originate',
    rfc: 'RFC 3101 — NSSA Option: Type 7 LSA, ABR translation',
  },
  totallynssa: {
    label:   'Totally NSSA',
    color:   '#8b5cf6',
    cmd:     'router ospf 1\n area 1 nssa no-summary\n! (ABR only — internal routers use "area 1 nssa")',
    allowed: ['T1','T2','T7'],
    blocked: ['T3','T4','T5'],
    hasDefault: true,
    hasT7:   true,
    lsdbInStub:  ['T1','T2','T7','T0'],
    rtEntries: [
      { prefix:'10.0.0.0/24',  type:'O',    cost:1,  via:'Direct'   },
      { prefix:'192.0.2.0/24', type:'O N2', cost:20, via:'R6(ASBR)' },
      { prefix:'0.0.0.0/0',    type:'O*IA', cost:1,  via:'R1(ABR)'  },
    ],
    desc: 'Totally NSSA — blocks Type 3, 4, and 5. Only one Type 3 default route enters. Local ASBR can still redistribute external routes as Type 7.\n\nMost scalable area type that still allows external redistribution. Combines the benefits of Totally Stub (minimal LSDB) with NSSA (local external redistribution).\n\nDefault route IS automatically injected by ABR when no-summary is used.',
    rfc: 'RFC 3101 + Cisco extension — no-summary + nssa = Totally NSSA',
  },
};
...
/* ═══════════════════════════════════════════════════════
   ENTRY POINT
   ═══════════════════════════════════════════════════════ */
function ospfAtcInit() {
  const host = document.getElementById('ospf-atc-container');
  if (!host) { console.error('Phase 6: #ospf-atc-container missing'); return; }
  host.innerHTML = atcBuildHTML();
  atcCanvas = document.getElementById('atcCanvas');
  atcCtx    = atcCanvas.getContext('2d');
  window.removeEventListener('resize', atcResize);
  window.addEventListener('resize', atcResize);
  atcSetType('standard');
  requestAnimationFrame(() => atcResize());
}

/* ═══════════════════════════════════════════════════════
   HTML + CSS
   ═══════════════════════════════════════════════════════ */
function atcBuildHTML() {
  return `
<style>
.atc-wrap{font-family:'IBM Plex Mono',monospace;background:#080d1c;color:#e2e8f0;border-radius:16px;padding:20px;border:1px solid #1a2640;box-shadow:0 8px 40px rgba(0,0,0,.6);user-select:none}
.atc-topbar{display:flex;flex-wrap:wrap;gap:10px;margin-bottom:14px;align-items:center;padding:12px 16px;background:#0d1526;border-radius:10px;border:1px solid #1a2640}
.atc-title{font-size:14px;font-weight:700;color:#60b8ff}
.atc-type-row{display:flex;flex-wrap:wrap;gap:8px;margin-bottom:14px}
.atc-type-btn{padding:8px 16px;border-radius:8px;border:2px solid #1e3050;background:#0d1829;color:#64748b;font-size:11px;font-family:'IBM Plex Mono',monospace;font-weight:700;cursor:pointer;transition:all .22s;letter-spacing:.3px}
.atc-type-btn:hover{transform:translateY(-2px);box-shadow:0 4px 16px rgba(0,0,0,.4)}
.atc-type-btn.active{transform:translateY(-2px)}
.atc-body{display:grid;grid-template-columns:1fr 320px;gap:14px}
.atc-canvas-box{background:#04080f;border-radius:12px;overflow:hidden;border:1px solid #1a2640;box-shadow:inset 0 2px 20px rgba(0,0,0,.4)}
#atcCanvas{display:block;width:100%;height:auto}
.atc-panel{background:#0d1526;border-radius:10px;padding:14px;border:1px solid #1a2640}
.atc-panel-title{font-size:9.5px;color:#2c4470;text-transform:uppercase;letter-spacing:1px;margin-bottom:10px;font-weight:700;display:flex;align-items:center;gap:6px}
.atc-panel-title::before{content:'';display:inline-block;width:3px;height:11px;border-radius:2px;background:var(--atc-ac,#3b82f6)}
/* LSA matrix */
.atc-matrix{width:100%;border-collapse:collapse;font-size:10px;margin-bottom:10px}
.atc-matrix th{background:#050c1a;color:#2c4470;padding:5px 8px;text-align:center;font-weight:700;border-bottom:1px solid #1a2640;font-size:9px;text-transform:uppercase;letter-spacing:.5px}
.atc-matrix td{padding:5px 8px;border-bottom:1px solid #080e1c;text-align:center;vertical-align:middle}
.atc-matrix td:first-child{text-align:left;font-weight:700}
.atc-pill{display:inline-flex;align-items:center;gap:4px;padding:2px 9px;border-radius:12px;font-size:9.5px;font-weight:700}
.atc-pill.allow{background:rgba(34,197,94,0.12);color:#22c55e;border:1px solid rgba(34,197,94,0.25)}
.atc-pill.block{background:rgba(239,68,68,0.10);color:#ef4444;border:1px solid rgba(239,68,68,0.2)}
/* LSDB list */
.atc-lsdb-box{background:#050c1a;border-radius:8px;padding:10px;margin-bottom:10px;border:1px solid #1a2640}
.atc-lsdb-entry{display:flex;align-items:center;gap:8px;padding:3px 0;font-size:10.5px;border-bottom:1px solid #0a1525;transition:all .2s}
.atc-lsdb-entry:last-child{border-bottom:none}
.atc-lsdb-badge{padding:1px 8px;border-radius:4px;font-size:9px;font-weight:700;min-width:38px;text-align:center}
/* routing table */
.atc-rt-box{background:#050c1a;border-radius:8px;padding:10px;margin-bottom:10px;border:1px solid #1a2640}
.atc-rt-entry{display:flex;align-items:center;gap:8px;padding:3px 0;font-size:10px;border-bottom:1px solid #0a1525}
.atc-rt-entry:last-child{border-bottom:none}
.atc-rt-type{padding:1px 7px;border-radius:4px;font-size:9px;font-weight:700;min-width:40px;text-align:center}
/* CLI */
.atc-cli{font-size:10px;color:#4ade80;background:#050c1a;padding:9px 12px;border-radius:7px;margin-bottom:10px;white-space:pre-wrap;border-left:3px solid #22c55e;line-height:1.7}
/* desc */
.atc-desc{font-size:11px;color:#8aa3c5;line-height:1.8;font-family:'DM Sans',sans-serif;white-space:pre-line}
.atc-rfc{font-size:10px;color:#2c4470;margin-top:6px;font-style:italic}
/* comparison bar at bottom */
.atc-cmp-bar{display:grid;grid-template-columns:repeat(5,1fr);gap:6px;margin-top:14px}
.atc-cmp-cell{background:#0d1526;border-radius:8px;padding:10px 8px;border:2px solid #1a2640;text-align:center;cursor:pointer;transition:all .2s}
.atc-cmp-cell:hover{border-color:#334155}
.atc-cmp-cell.active-cmp{transform:translateY(-3px);box-shadow:0 6px 20px rgba(0,0,0,.5)}
.atc-cmp-cell-title{font-size:10px;font-weight:700;margin-bottom:6px}
.atc-cmp-lsa{font-size:8.5px;padding:1px 5px;border-radius:3px;font-weight:700;display:inline-block;margin:1px}
@media(max-width:800px){.atc-body{grid-template-columns:1fr}.atc-cmp-bar{grid-template-columns:repeat(3,1fr)}}
</style>

<div class="atc-wrap">
  <div class="atc-topbar">
    <span class="atc-title">🏗 Area Type Comparator</span>
    <span style="font-size:10px;color:#3b5278;margin-left:4px">RFC 2328 §3.6 · RFC 3101 · Click any type to compare</span>
  </div>

  <!-- Type selector -->
  <div class="atc-type-row" id="atcTypeRow"></div>

  <div class="atc-body">
    <!-- Canvas -->
    <div>
      <div class="atc-canvas-box">
        <canvas id="atcCanvas" width="680" height="340"></canvas>
      </div>
      <!-- comparison bar -->
      <div class="atc-cmp-bar" id="atcCmpBar"></div>
    </div>

    <!-- Right panel -->
    <div style="display:flex;flex-direction:column;gap:10px">
      <!-- LSA matrix -->
      <div class="atc-panel" style="--atc-ac:#3b82f6">
        <div class="atc-panel-title">LSA Flooding Matrix</div>
        <table class="atc-matrix">
          <thead><tr><th>LSA Type</th><th>Name</th><th>Status</th></tr></thead>
          <tbody id="atcMatrix"></tbody>
        </table>
      </div>

      <!-- LSDB in stub router -->
      <div class="atc-panel" style="--atc-ac:#f59e0b">
        <div class="atc-panel-title">LSDB Contents (stub area router)</div>
        <div class="atc-lsdb-box" id="atcLsdb"></div>
      </div>

      <!-- Routing table -->
      <div class="atc-panel" style="--atc-ac:#22c55e">
        <div class="atc-panel-title">Routing Table (stub area router)</div>
        <div class="atc-rt-box" id="atcRt"></div>
      </div>

      <!-- Config -->
      <div class="atc-panel" style="--atc-ac:#8b5cf6">
        <div class="atc-panel-title">IOS Configuration</div>
        <div class="atc-cli" id="atcCli"></div>
      </div>

      <!-- Description -->
      <div class="atc-panel" style="--atc-ac:#06b6d4">
        <div class="atc-panel-title">Explanation</div>
        <div class="atc-desc" id="atcDesc"></div>
        <div class="atc-rfc" id="atcRfc"></div>
      </div>
    </div>
  </div>
</div>`;
...
/* ═══════════════════════════════════════════════════════
   SET AREA TYPE — main controller
   ═══════════════════════════════════════════════════════ */
function atcSetType(type) {
  atcAreaType = type;
  const at = ATC_AREA_TYPES[type];

  // ── Type buttons ──
  const row = document.getElementById('atcTypeRow');
  row.innerHTML = Object.entries(ATC_AREA_TYPES).map(([k,v]) => {
    const active = k === type;
    return `<button class="atc-type-btn${active?' active':''}"
      style="${active?`border-color:${v.color};background:${v.color}18;color:${v.color};box-shadow:0 0 14px ${v.color}33`:''}"
      onclick="atcSetType('${k}')">${v.label}</button>`;
  }).join('');

  // ── LSA matrix ──
  const tbody = document.getElementById('atcMatrix');
  tbody.innerHTML = ATC_LSA_TYPES.map(l => {
    const allowed = at.allowed.includes(l.key) || (l.key==='T0'&&at.hasDefault) || (l.key==='T7'&&at.hasT7);
    const cls     = allowed ? 'allow' : 'block';
    const icon    = allowed ? '✔ Allowed' : '✘ Blocked';
    return `<tr>
      <td><span style="color:${l.color};font-weight:700">${l.label}</span></td>
      <td style="color:#64748b;font-size:10px">${l.name}</td>
      <td><span class="atc-pill ${cls}">${icon}</span></td>
    </tr>`;
  }).join('');

  // ── LSDB ──
  const lsdb = document.getElementById('atcLsdb');
  lsdb.innerHTML = at.lsdbInStub.map(key => {
    const l = ATC_LSA_TYPES.find(x => x.key===key) || { color:'#a78bfa', name:'Default Route', label:'Default' };
    return `<div class="atc-lsdb-entry">
      <span class="atc-lsdb-badge" style="background:${l.color}22;color:${l.color};border:1px solid ${l.color}44">${l.label}</span>
      <span style="color:#8aa3c5;font-size:10px">${l.name}${key==='T0'?' (0.0.0.0/0 via ABR)':''}</span>
    </div>`;
  }).join('');

  // ── Routing Table ──
  const rtBox = document.getElementById('atcRt');
  const rtTypeColors = { 'O':'#3b82f6','O IA':'#22c55e','O E2':'#ef4444','O E1':'#f87171','O N2':'#06b6d4','O N1':'#22d3ee','O*IA':'#a78bfa' };
  rtBox.innerHTML = at.rtEntries.map(r => {
    const c = rtTypeColors[r.type] || '#64748b';
    return `<div class="atc-rt-entry">
      <span class="atc-rt-type" style="background:${c}22;color:${c};border:1px solid ${c}44">${r.type}</span>
      <span style="color:#7db8ff;font-size:10.5px">${r.prefix}</span>
      <span style="color:#3b5278;font-size:10px">[110/${r.cost}]</span>
      <span style="color:#4a6a90;font-size:10px;margin-left:auto">via ${r.via}</span>
    </div>`;
  }).join('');

  // ── CLI ──
  document.getElementById('atcCli').textContent = at.cmd;

  // ── Desc / RFC ──
  document.getElementById('atcDesc').textContent = at.desc;
  document.getElementById('atcRfc').textContent  = at.rfc;

  // ── Comparison bar ──
  const cmpBar = document.getElementById('atcCmpBar');
  cmpBar.innerHTML = Object.entries(ATC_AREA_TYPES).map(([k,v]) => {
    const active = k === type;
    const lsas   = v.lsdbInStub.slice(0,4);
    return `<div class="atc-cmp-cell${active?' active-cmp':''}"
      style="${active?`border-color:${v.color};background:${v.color}10`:''}"
      onclick="atcSetType('${k}')">
      <div class="atc-cmp-cell-title" style="color:${v.color}">${v.label}</div>
      <div>${lsas.map(l=>{const lt=ATC_LSA_TYPES.find(x=>x.key===l)||{color:'#a78bfa',label:'Def'};return `<span class="atc-cmp-lsa" style="background:${lt.color}22;color:${lt.color}">${lt.label}</span>`;}).join('')}</div>
      <div style="font-size:9px;color:#3b5278;margin-top:4px">${v.rtEntries.length} routes</div>
    </div>`;
  }).join('');
...
function atcResize() {
  if (!atcCanvas) return;
  const w = atcCanvas.parentElement.clientWidth;
  atcCanvas.style.width  = w + 'px';
  atcCanvas.style.height = Math.round(w * 0.50) + 'px';
  atcDraw();
}

function atcDraw() {
  if (!atcCtx) return;
  const W = atcCanvas.width  = atcCanvas.parentElement.clientWidth;
  const H = atcCanvas.height = Math.round(W * 0.50);
  const ctx = atcCtx;
  ctx.clearRect(0,0,W,H);
  ...
  const at  = ATC_AREA_TYPES[atcAreaType];
  const ac  = at.color;

  // ── Area zones ──
  // Area 0 (left 48%)
  atcAreaZone(ctx, 0, 0, Math.round(W*0.48), H, '#3b82f6', 'Area 0 — Backbone');
  // Stub area (right 52%)
  atcAreaZone(ctx, Math.round(W*0.48), 0, Math.round(W*0.52), H, ac, `${at.label}\n(Area 1)`);

  const mY = Math.round(H * 0.42);
  const lY = Math.round(H * 0.72);

  // Router positions
  const pos = {
    R1: { x: Math.round(W*0.20), y: mY },   // Area 0 router
    ABR:{ x: Math.round(W*0.48), y: mY },   // ABR (boundary)
    R2: { x: Math.round(W*0.68), y: mY },   // stub area IR
    R3: { x: Math.round(W*0.84), y: mY },   // stub area ASBR (only NSSA)
  };

  // ── Links ──
  // R1 — ABR
  atcDrawLink(ctx, pos.R1, pos.ABR, '#3b82f6', true, 'Cost 1');
  // ABR — R2
  atcDrawLink(ctx, pos.ABR, pos.R2, ac, true, 'Cost 1');
  // ABR — R3 (only for NSSA types)
  if (at.hasT7) {
    atcDrawLink(ctx, pos.ABR, pos.R3, ac, true, 'Cost 1');
    atcDrawLink(ctx, pos.R3, { x: pos.R3.x + 48, y: pos.R3.y }, ac, false, '');
    ctx.fillStyle = ac + '88'; ctx.font = '9px IBM Plex Mono'; ctx.textAlign = 'left';
    ctx.fillText('External route', pos.R3.x + 54, pos.R3.y + 4);
  }

  // ── LSA flow arrows ── (from left side into area)
  const arrowX = Math.round(W * 0.48);
  let arrowY = Math.round(H * 0.20);
  const arrowStep = 22;
  ATC_LSA_TYPES.filter(l => l.key !== 'T0').forEach(l => {
    const isAllowed = at.allowed.includes(l.key) || (l.key==='T7' && at.hasT7);
    const isDefault = l.key === 'T0';
    if (isDefault) return;
    atcDrawLsaArrow(ctx, arrowX, arrowY, isAllowed, l.color, l.label, at);
    arrowY += arrowStep;
  });
  // Default route arrow (always from ABR into area if stub)
  if (at.hasDefault) {
    const defL = ATC_LSA_TYPES.find(x=>x.key==='T0');
    atcDrawDefaultArrow(ctx, arrowX, arrowY, defL.color);
  }

  // ── Routers ──
  atcDrawRouter(ctx, pos.R1,  'R1',  'IR',   '#3b82f6', 'Area 0');
  atcDrawRouter(ctx, pos.ABR, 'ABR', 'ABR',  ac,        'ABR');
  atcDrawRouter(ctx, pos.R2,  'R2',  'IR',   ac,        at.label);
  if (at.hasT7) {
    atcDrawRouter(ctx, pos.R3, 'R3', 'ASBR', ac, 'ASBR in NSSA');
  }

  // ── Area type label ──
  ctx.fillStyle = ac; ctx.font = 'bold 13px IBM Plex Mono'; ctx.textAlign = 'center';
  ctx.fillText(at.label, Math.round(W*0.74), H - 16);
}

function atcAreaZone(ctx, x, y, w, h, color, label) {
  ctx.fillStyle   = color + '0d';
  ctx.strokeStyle = color + '2e';
  ctx.lineWidth   = 1.5;
  ctx.setLineDash([6,4]);
...
function atcDrawLink(ctx, a, b, color, solid, label) {
  ...
  if (label) {
    ...
    ctx.fillStyle = '#3b5278'; ctx.font = '9px IBM Plex Mono'; ctx.textAlign = 'center';
    ctx.fillText(label, mx, my);
  }
}

function atcDrawLsaArrow(ctx, x, y, allowed, color, label, at) {
  const arrowLen = 36;
  const ax = x - (allowed ? arrowLen : 18);
  // line
  ctx.strokeStyle = color + (allowed ? 'cc' : '44');
  ctx.lineWidth   = 1.5;
  ctx.setLineDash([]);
  ctx.beginPath(); ctx.moveTo(ax, y); ctx.lineTo(x, y); ctx.stroke();
  // arrowhead or block
  if (allowed) {
    ctx.fillStyle = color + 'cc';
    ctx.beginPath();
    ctx.moveTo(x+8, y); ctx.lineTo(x, y-4); ctx.lineTo(x, y+4);
  ...
  // label
  ctx.fillStyle = color + (allowed ? 'ff' : '55');
  ctx.font = 'bold 9px IBM Plex Mono'; ctx.textAlign = 'right';
  ctx.fillText(label, ax - 4, y + 3);
}

function atcDrawDefaultArrow(ctx, x, y, color) {
  ...
  ctx.fillText('Default (T0/T3)', x - 4, y + 3);
}

function atcDrawRouter(ctx, p, id, role, color, areaLabel) {
  const RW = 90, RH = 28;
  const rx  = p.x - RW/2, ry = p.y - RH/2;
  const bg  = ctx.createLinearGradient(rx, ry, rx, ry+RH);
  bg.addColorStop(0, '#0d1f3a'); bg.addColorStop(1, '#080f1e');
  ...
  ctx.lineWidth   = role === 'ABR' ? 2.5 : 1.5;
  if (role === 'ABR') { ctx.shadowColor = color + '66'; ctx.shadowBlur = 14; }
  lsaRR(ctx, rx, ry, RW, RH, 8); ctx.fill(); ctx.stroke();
  ctx.shadowBlur = 0;
  // accent stripe
  const sg = ctx.createLinearGradient(rx,ry,rx+RW,ry);
  sg.addColorStop(0, color+'00'); sg.addColorStop(0.5, color+'55'); sg.addColorStop(1, color+'00');
  ctx.fillStyle = sg;
  ctx.beginPath(); ctx.roundRect(rx, ry, RW, 3, [8,8,0,0]); ctx.fill();
  // text
  ctx.fillStyle = '#ddeeff'; ctx.font = 'bold 11px IBM Plex Mono'; ctx.textAlign = 'center';
  ctx.fillText(id, p.x, p.y - 3);
  ctx.fillStyle = color + '99'; ctx.font = 'bold 8px IBM Plex Mono';
  ctx.fillText('[' + role + ']', p.x, p.y + 9);
  // area label below
  ctx.fillStyle = '#2c4470'; ctx.font = '9px IBM Plex Mono';
  ctx.fillText(areaLabel, p.x, ry + RH + 14);
}

/* ╔══════════════════════════════════════════════════════════════════╗
   ║  OSPF SIMULATOR · Phase 7 : Path Selection                      ║
   ║  O · O IA · O E1 · O E2 · O N1 · O N2 · Interactive           ║
   ╚══════════════════════════════════════════════════════════════════╝ */

/* ═══════════════════════════════════════════════════════
   PHASE 7 STATE
   ═══════════════════════════════════════════════════════ */
let pselCanvas, pselCtx;
let pselScenario  = 'preference';
let pselUserCosts = { O:1, OIA:2, OE1:25, OE2:20, ON1:30, ON2:20 };
let pselAnimId    = null;
let pselHighlight = null;  // currently highlighted route type

const PSEL_ROUTE_TYPES = [
  { key:'O',   label:'O',    name:'Intra-Area',          color:'#22c55e', ad:110, pref:1,
    desc:'Intra-area route — learned within the same OSPF area via Type 1/2 LSAs. Always preferred over inter-area routes regardless of cost. OSPF protects loop-free intra-area topology using SPF.' },
  { key:'OIA', label:'O IA', name:'Inter-Area',          color:'#3b82f6', ad:110, pref:2,
    desc:'Inter-area route — learned from a different area via Type 3 Summary LSAs. ABRs generate Type 3s. Always preferred over external routes, never over intra-area.' },
  { key:'OE1', label:'O E1', name:'External Type 1',     color:'#f59e0b', ad:110, pref:3,
    desc:'External Type 1 — metric includes both the external metric AND the internal OSPF cost to reach the ASBR. Total cost = internal cost to ASBR + external seed metric. Preferred over E2 when costs are equal or lower.' },
  { key:'OE2', label:'O E2', name:'External Type 2',     color:'#f97316', ad:110, pref:4,
    desc:'External Type 2 — metric is the external seed metric only. Internal cost to ASBR is NOT added. All routers see the same cost regardless of distance to ASBR. Default for redistribute. Tiebreak via lowest cost to ASBR.' },
  { key:'ON1', label:'O N1', name:'NSSA External Type 1',color:'#06b6d4', ad:110, pref:5,
    desc:'NSSA External Type 1 — same as E1 but from a Type 7 LSA in an NSSA area. Total cost = internal NSSA cost + external metric. Preferred over N2. Used when the NSSA ASBR redistributes with E1 metric type.' },
  { key:'ON2', label:'O N2', name:'NSSA External Type 2',color:'#0ea5e9', ad:110, pref:6,
    desc:'NSSA External Type 2 — external metric only, from Type 7 LSA. Least preferred OSPF route type. Same prefix installed as N2 and E2 — N2 loses to E2. Used in NSSA areas when external metric type is E2 (default).' },
];

/* ── Preference rules ── */
const PSEL_PREF_RULES = [
  'Intra-area (O) beats ALL other types regardless of cost',
  'Inter-area (O IA) beats all external types regardless of cost',
  'External Type 1 (O E1) beats E2 for the same prefix',
  'External Type 2 (O E2) beats NSSA types (N1, N2)',
  'NSSA Type 1 (O N1) beats N2 for the same prefix',
  'NSSA Type 2 (O N2) is least preferred OSPF route',
  'E1 tiebreak: lower total cost wins (internal + external)',
  'E2 tiebreak: lower cost to ASBR wins (equal external metric)',
  'AD 110 for all OSPF types — AD never tiebreaks within OSPF',
];

/* ── Scenarios ── */
const PSEL_SCENARIOS = {
  preference: {
    label: 'Preference Order',
    desc:  'Same destination prefix learned via all 6 OSPF route types simultaneously. The router selects the BEST route based on the strict preference order. Adjust costs using the sliders — the winner never changes based on cost alone.',
  },
  e1vse2: {
    label: 'E1 vs E2 Deep Dive',
    desc:  'Two ASBRs redistribute the same prefix. One uses E1 (adds internal cost), the other uses E2 (external metric only). Shows why E1 is preferred and when it matters for traffic engineering.',
  },
  intravsinter: {
    label: 'Intra vs Inter',
    desc:  'Same prefix exists in Area 0 (intra-area for R1) and is also advertised as a Type 3 Summary from an ABR. R1 always installs the intra-area route — even if the inter-area cost is lower.',
  },
  ecmp: {
    label: 'ECMP & Tiebreaking',
    desc:  'Two equal-cost paths to the same prefix (E2, same external metric). OSPF uses the cost to the ASBR as tiebreak. If that is also equal, both paths are installed (ECMP up to maximum-paths).',
  },
};
...
/* ═══════════════════════════════════════════════════════
   ENTRY POINT
   ═══════════════════════════════════════════════════════ */
function ospfPselInit() {
  const host = document.getElementById('ospf-psel-container');
  if (!host) { console.error('Phase 7: #ospf-psel-container missing'); return; }
  host.innerHTML = pselBuildHTML();
  pselCanvas = document.getElementById('pselCanvas');
  pselCtx    = pselCanvas.getContext('2d');
  window.removeEventListener('resize', pselResize);
  window.addEventListener('resize', pselResize);
  pselLoadScenario('preference');
  requestAnimationFrame(() => pselResize());
}

/* ═══════════════════════════════════════════════════════
   HTML + CSS
   ═══════════════════════════════════════════════════════ */
function pselBuildHTML() {
  return `
<style>
.psel-wrap{font-family:'IBM Plex Mono',monospace;background:#080d1c;color:#e2e8f0;border-radius:16px;padding:20px;border:1px solid #1a2640;box-shadow:0 8px 40px rgba(0,0,0,.6);user-select:none}
.psel-topbar{display:flex;flex-wrap:wrap;gap:10px;margin-bottom:14px;align-items:center;padding:12px 16px;background:#0d1526;border-radius:10px;border:1px solid #1a2640}
.psel-title{font-size:14px;font-weight:700;color:#60b8ff}
.psel-scen-row{display:flex;flex-wrap:wrap;gap:6px;margin-bottom:12px}
.psel-btn{background:#0d1829;border:1px solid #1e3050;color:#64748b;padding:6px 14px;border-radius:7px;cursor:pointer;font-size:11px;font-family:'IBM Plex Mono',monospace;font-weight:600;transition:all .2s}
.psel-btn:hover{border-color:#3b82f6;color:#7cb9ff;background:#0f1f3a}
.psel-btn.active{background:linear-gradient(135deg,#0f2a52,#112244);border-color:#3b82f6;color:#7cb9ff;font-weight:700}
.psel-desc-bar{font-size:11px;color:#64748b;background:#0d1526;border-radius:8px;padding:9px 13px;margin-bottom:12px;border:1px solid #1a2640;line-height:1.6;font-family:'DM Sans',sans-serif}
.psel-body{display:grid;grid-template-columns:1fr 300px;gap:14px}
.psel-canvas-box{background:#04080f;border-radius:12px;overflow:hidden;border:1px solid #1a2640;box-shadow:inset 0 2px 20px rgba(0,0,0,.4);margin-bottom:12px}
#pselCanvas{display:block;width:100%;height:auto}
.psel-panel{background:#0d1526;border-radius:10px;padding:14px;border:1px solid #1a2640;margin-bottom:10px}
.psel-panel-title{font-size:9.5px;color:#2c4470;text-transform:uppercase;letter-spacing:1px;margin-bottom:10px;font-weight:700;display:flex;align-items:center;gap:6px}
.psel-panel-title::before{content:'';display:inline-block;width:3px;height:11px;border-radius:2px;background:var(--psel-ac,#3b82f6)}
/* Route type cards */
.psel-route-card{border-radius:9px;padding:11px 13px;margin-bottom:8px;border:2px solid transparent;cursor:pointer;transition:all .2s;position:relative;overflow:hidden}
.psel-route-card:hover{transform:translateX(3px)}
.psel-route-card.winner{border-color:var(--rc) !important;box-shadow:0 0 20px var(--rc-glow)}
.psel-route-card.winner::before{content:'🏆 BEST PATH';position:absolute;top:6px;right:8px;font-size:8px;font-weight:700;color:var(--rc);letter-spacing:.5px}
.psel-rt-label{font-size:13px;font-weight:700;margin-bottom:2px}
.psel-rt-name{font-size:10px;color:#4a6a90;margin-bottom:6px}
.psel-rt-cost{display:flex;align-items:center;gap:10px;margin-bottom:4px}
.psel-rt-cost label{font-size:10px;color:#3b5278;min-width:55px}
.psel-rt-cost input[type=range]{flex:1;height:3px;appearance:none;background:#1a2640;border-radius:2px;cursor:pointer;outline:none}
.psel-rt-cost input[type=range]::-webkit-slider-thumb{appearance:none;width:12px;height:12px;border-radius:50%;cursor:pointer;transition:transform .15s}
.psel-rt-cost input[type=range]::-webkit-slider-thumb:hover{transform:scale(1.3)}
.psel-rt-cost .cost-val{font-size:11px;font-weight:700;min-width:28px;text-align:right}
/* Preference order visual */
.psel-pref-list{display:flex;flex-direction:column;gap:5px}
.psel-pref-item{display:flex;align-items:center;gap:8px;padding:6px 10px;border-radius:7px;background:#060d1a;border:1px solid #1a2640;font-size:11px;transition:all .2s}
.psel-pref-item.active-pref{border-left:3px solid var(--pc);background:#0a1525;padding-left:8px}
.psel-pref-rank{font-size:10px;color:#2c4470;min-width:16px;text-align:center;font-weight:700}
.psel-pref-badge{padding:2px 8px;border-radius:4px;font-size:10px;font-weight:700;min-width:44px;text-align:center}
/* Rules */
.psel-rule-item{font-size:10.5px;color:#8aa3c5;line-height:1.7;padding:3px 0;border-bottom:1px solid #0a1525;display:flex;align-items:flex-start;gap:6px;font-family:'DM Sans',sans-serif}
.psel-rule-item:last-child{border-bottom:none}
.psel-rule-item::before{content:'▸';color:#3b82f6;flex-shrink:0;margin-top:2px}
/* winner display */
.psel-winner-box{background:rgba(34,197,94,0.07);border:1.5px solid rgba(34,197,94,0.25);border-radius:9px;padding:12px;margin-bottom:10px;display:none}
.psel-winner-title{font-size:10px;color:#22c55e;font-weight:700;text-transform:uppercase;letter-spacing:.8px;margin-bottom:6px}
.psel-winner-route{font-size:13px;font-weight:700;margin-bottom:4px}
.psel-winner-why{font-size:11px;color:#8aa3c5;line-height:1.7;font-family:'DM Sans',sans-serif}
.psel-cli{font-size:10px;color:#4ade80;background:#050c1a;padding:9px 12px;border-radius:7px;white-space:pre-wrap;border-left:3px solid #22c55e;line-height:1.7;margin-top:8px;max-height:120px;overflow-y:auto}
@media(max-width:820px){.psel-body{grid-template-columns:1fr}}
</style>

<div class="psel-wrap">
  <div class="psel-topbar">
    <span class="psel-title">⚖ OSPF Path Selection</span>
    <span style="font-size:10px;color:#3b5278;margin-left:4px">RFC 2328 §16 — Route preference: O > O IA > E1 > E2 > N1 > N2</span>
  </div>

  <div class="psel-scen-row" id="pselScenRow"></div>
  <div class="psel-desc-bar" id="pselDescBar"></div>

  <div class="psel-body">
    <!-- Left: canvas + route cards -->
    <div>
      <div class="psel-canvas-box">
        <canvas id="pselCanvas" width="720" height="320"></canvas>
      </div>

      <!-- Winner box -->
      <div class="psel-winner-box" id="pselWinnerBox">
        <div class="psel-winner-title">🏆 Best Path Selected</div>
        <div class="psel-winner-route" id="pselWinnerRoute"></div>
        <div class="psel-winner-why"   id="pselWinnerWhy"></div>
        <div class="psel-cli"          id="pselWinnerCli"></div>
      </div>

      <!-- Route type cards with sliders -->
      <div id="pselRouteCards"></div>
    </div>

    <!-- Right: preference list + rules -->
    <div>
      <div class="psel-panel" style="--psel-ac:#22c55e">
        <div class="psel-panel-title">Preference Order (Highest → Lowest)</div>
        <div class="psel-pref-list" id="pselPrefList"></div>
      </div>

      <div class="psel-panel" style="--psel-ac:#3b82f6">
        <div class="psel-panel-title">Selection Rules</div>
        <div id="pselRules">
          ${PSEL_PREF_RULES.map(r=>`<div class="psel-rule-item">${r}</div>`).join('')}
        </div>
      </div>

      <div class="psel-panel" style="--psel-ac:#f59e0b">
        <div class="psel-panel-title">E1 vs E2 Cost Model</div>
        <div style="font-size:10.5px;color:#8aa3c5;line-height:1.85;font-family:'DM Sans',sans-serif">
          <div style="margin-bottom:6px"><span style="color:#f59e0b;font-weight:700">E1 total cost</span> = internal cost to ASBR <span style="color:#3b5278">+</span> external metric</div>
          <div style="margin-bottom:6px"><span style="color:#f97316;font-weight:700">E2 total cost</span> = external metric only (fixed)</div>
          <div style="margin-bottom:6px"><span style="color:#06b6d4;font-weight:700">N1 total cost</span> = internal NSSA cost + external metric</div>
          <div><span style="color:#0ea5e9;font-weight:700">N2 total cost</span> = external metric only (in NSSA)</div>
        </div>
      </div>
    </div>
  </div>
...
/* ═══════════════════════════════════════════════════════
   SCENARIO LOAD
   ═══════════════════════════════════════════════════════ */
function pselLoadScenario(key) {
  pselScenario = key;
  const sc = PSEL_SCENARIOS[key];

  // Buttons
  const row = document.getElementById('pselScenRow');
  row.innerHTML = Object.entries(PSEL_SCENARIOS).map(([k,s]) =>
    `<button class="psel-btn${k===key?' active':''}" onclick="pselLoadScenario('${k}')">${s.label}</button>`
  ).join('');

  document.getElementById('pselDescBar').innerHTML = `<b>${sc.label}:</b> ${sc.desc}`;

  // Set default costs per scenario
  if (key === 'preference') {
    pselUserCosts = { O:1, OIA:2, OE1:25, OE2:20, ON1:30, ON2:20 };
  } else if (key === 'e1vse2') {
    pselUserCosts = { O:999, OIA:999, OE1:15, OE2:20, ON1:999, ON2:999 };
  } else if (key === 'intravsinter') {
    pselUserCosts = { O:10, OIA:2, OE1:999, OE2:999, ON1:999, ON2:999 };
  } else if (key === 'ecmp') {
    pselUserCosts = { O:999, OIA:999, OE1:999, OE2:20, ON1:999, ON2:20 };
  }
...
/* ═══════════════════════════════════════════════════════
   RENDER ROUTE CARDS WITH SLIDERS
   ═══════════════════════════════════════════════════════ */
function pselRenderCards() {
  const winner = pselGetWinner();
  const cards  = document.getElementById('pselRouteCards');
  cards.innerHTML = PSEL_ROUTE_TYPES.map(rt => {
    const isWinner = winner && winner.key === rt.key;
    const cost     = pselUserCosts[rt.key] || 1;
    const active   = cost < 999;
    return `
    <div class="psel-route-card${isWinner?' winner':''}"
      style="background:${rt.color}${isWinner?'18':'0d'};border-color:${isWinner?rt.color:'#1e3050'};
             --rc:${rt.color};--rc-glow:${rt.color}44"
      onmouseenter="pselHighlightType('${rt.key}')"
      onmouseleave="pselHighlightType(null)">
      <div class="psel-rt-label" style="color:${rt.color}">${rt.label}</div>
      <div class="psel-rt-name">${rt.name}</div>
      <div class="psel-rt-cost">
        <label>Cost</label>
        <input type="range" min="1" max="200" value="${Math.min(cost,200)}"
          style="--tc:${rt.color}"
          oninput="pselUpdateCost('${rt.key}',this.value)"
          ${pselScenario!=='preference'&&cost>200?'disabled':''}>
        <span class="cost-val" style="color:${rt.color}" id="pselCostVal_${rt.key}">${active?cost:'N/A'}</span>
      </div>
      <div style="font-size:10px;color:#3b5278;margin-top:3px">
        AD: 110 · Pref rank: #${rt.pref} · ${isWinner?'<span style="color:'+rt.color+';font-weight:700">✔ INSTALLED</span>':'Not selected'}
      </div>
    </div>`;
  }).join('');

  // Style slider thumbs
  document.querySelectorAll('.psel-route-card input[type=range]').forEach(input => {
    input.style.setProperty('--tc', input.closest('.psel-route-card').style.getPropertyValue('--rc'));
    input.style.background = '#1a2640';
  });
}

function pselUpdateCost(key, val) {
  pselUserCosts[key] = parseInt(val);
  document.getElementById('pselCostVal_'+key).textContent = val;
  pselUpdateAll();
}

function pselHighlightType(key) {
  pselHighlight = key;
  pselDraw();
}
...
/* ═══════════════════════════════════════════════════════
   WINNER CALCULATION
   ═══════════════════════════════════════════════════════ */
function pselGetWinner() {
  // Filter routes that are "active" (cost < 999 = present in routing table)
  const active = PSEL_ROUTE_TYPES.filter(rt => (pselUserCosts[rt.key]||999) < 999);
  if (!active.length) return null;
  // Sort by preference (lower = better)
  active.sort((a,b) => a.pref - b.pref);
  return active[0];
}

function pselUpdateAll() {
  const winner = pselGetWinner();
  pselRenderCards();
  pselRenderPrefList(winner);
  pselRenderWinnerBox(winner);
  pselDraw();
}

function pselRenderPrefList(winner) {
  const list = document.getElementById('pselPrefList');
  list.innerHTML = PSEL_ROUTE_TYPES.map(rt => {
    const cost    = pselUserCosts[rt.key] || 999;
    const active  = cost < 999;
    const isWin   = winner && winner.key === rt.key;
    return `<div class="psel-pref-item${isWin?' active-pref':''}" style="--pc:${rt.color};opacity:${active?1:0.4}">
      <span class="psel-pref-rank" style="color:${isWin?rt.color:'#2c4470'}">#${rt.pref}</span>
      <span class="psel-pref-badge" style="background:${rt.color}22;color:${rt.color};border:1px solid ${rt.color}44">${rt.label}</span>
      <span style="font-size:10px;color:${isWin?rt.color:'#3b5278'};font-weight:${isWin?700:400}">${rt.name}</span>
      ${isWin?`<span style="color:${rt.color};font-size:10px;margin-left:auto;font-weight:700">WIN</span>`:''}
    </div>`;
  }).join('');
}

function pselRenderWinnerBox(winner) {
  const box = document.getElementById('pselWinnerBox');
  if (!winner) { box.style.display = 'none'; return; }
  box.style.display = 'block';
  box.style.borderColor = winner.color + '55';
  box.style.background  = winner.color + '08';

  document.getElementById('pselWinnerRoute').innerHTML =
    `<span style="color:${winner.color}">${winner.label} ${winner.name}</span> — Cost ${pselUserCosts[winner.key]}`;

  const whyMap = {
    O:   'Intra-area route wins unconditionally. Even if cost is higher than all other types, intra-area is always preferred. This ensures loop-free routing within an area.',
    OIA: 'Inter-area route wins because no intra-area route exists for this prefix. O IA is always preferred over any external type regardless of cost difference.',
    OE1: 'E1 wins because no O or O IA routes exist for this prefix. E1 is preferred over E2 because it includes the internal cost to the ASBR, giving a more accurate total path cost.',
    OE2: 'E2 wins because no higher-preference routes exist. E2 external metric is fixed — all routers see the same cost regardless of ASBR distance.',
    ON1: 'N1 wins from NSSA-originated external routes. N1 is preferred over N2 and both lose to E1/E2 if the same prefix exists as a Type 5.',
    ON2: 'N2 is the least preferred OSPF route type. It is only selected when no other OSPF route type exists for this prefix.',
  };

  document.getElementById('pselWinnerWhy').textContent = whyMap[winner.key] || '';
  document.getElementById('pselWinnerCli').textContent =
    `show ip route 192.0.2.0\n! Routing entry for 192.0.2.0/24\n!   Known via "${winner.label}", distance 110, metric ${pselUserCosts[winner.key]}\n!   Last update from <ASBR-IP>\n\nshow ip ospf border-routers\n! Identifies ASBR reachability for external route calculation`;
}
...
/* ═══════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════ */
function pselResize() {
  if (!pselCanvas) return;
  const w = pselCanvas.parentElement.clientWidth;
  pselCanvas.style.width  = w + 'px';
  pselCanvas.style.height = Math.round(w * 0.44) + 'px';
  pselDraw();
}

function pselDraw() {
  if (!pselCtx) return;
  const W = pselCanvas.width  = pselCanvas.parentElement.clientWidth;
  const H = pselCanvas.height = Math.round(W * 0.44);
  const ctx = pselCtx;
  ctx.clearRect(0,0,W,H);
  ...
  const destX = Math.round(W * 0.82);
  const destY = Math.round(H * 0.50);
  const srcX  = Math.round(W * 0.12);
  const srcY  = Math.round(H * 0.50);

  // Draw all route paths as parallel lines
  const activeRoutes = PSEL_ROUTE_TYPES.filter(rt => (pselUserCosts[rt.key]||999) < 999);
  const n = activeRoutes.length;
  const spread = Math.min(50, (H * 0.75) / Math.max(n, 1));

  activeRoutes.forEach((rt, i) => {
    const yOff = (i - (n-1)/2) * spread;
    const ay = srcY + yOff;
    const by = destY + yOff;
    const isWin  = winner && winner.key === rt.key;
    const isHL   = pselHighlight === rt.key;
    const cost   = pselUserCosts[rt.key];
    const alpha  = isWin ? 1 : isHL ? 0.8 : 0.35;

    // glow for winner
    if (isWin) { ctx.shadowColor = rt.color + 'aa'; ctx.shadowBlur = 16; }

    // path line
    ctx.strokeStyle = rt.color + Math.round(alpha * 255).toString(16).padStart(2,'0');
    ctx.lineWidth   = isWin ? 3.5 : isHL ? 2 : 1.5;
    ctx.setLineDash(isWin ? [] : [5,4]);
    ctx.beginPath();
    // bezier curve for visual clarity
    const cx1 = srcX + (destX - srcX) * 0.35;
    const cx2 = srcX + (destX - srcX) * 0.65;
    ctx.moveTo(srcX + 46, ay);
    ctx.bezierCurveTo(cx1, ay, cx2, by, destX - 46, by);
    ctx.stroke();
    ctx.setLineDash([]); ctx.shadowBlur = 0;

    // label on path
    const lx = srcX + (destX - srcX) * 0.50;
    const ly = (ay + by) / 2 - (isWin ? 14 : 10);
    ctx.fillStyle = rt.color + (isWin ? 'ff' : Math.round(alpha*200).toString(16).padStart(2,'0'));
    ctx.font = `${isWin?'bold ':''}${isWin?10:9}px IBM Plex Mono`;
    ctx.textAlign = 'center';
    ctx.fillText(`${rt.label}  cost ${cost}`, lx, ly);
  });

  // ── Source router ──
  pselDrawNode(ctx, srcX, srcY, 'Router', '#3b82f6', 'Computing\nbest path');

  // ── Destination ──
  pselDrawNode(ctx, destX, destY, '192.0.2.0/24', '#64748b', 'Destination\nprefix');

  // ── Winner arrow ──
  if (winner) {
    const winRoute = activeRoutes.find(r => r.key === winner.key);
    if (winRoute) {
      const i    = activeRoutes.indexOf(winRoute);
      const yOff = (i - (n-1)/2) * spread;
      const ay   = srcY + yOff;
      const by   = destY + yOff;
      ...
      ctx.shadowColor = winner.color; ctx.shadowBlur = 12;
      ctx.beginPath();
  ...
  // ── Legend ──
  ctx.fillStyle = '#2c4470'; ctx.font = '10px IBM Plex Mono'; ctx.textAlign = 'left';
  ctx.fillText('Solid = selected  ╌ Dashed = not selected  Glow = winner', 12, H - 12);
}

function pselDrawNode(ctx, x, y, label, color, sub) {
  const RW = 90, RH = 36;
  const rx = x - RW/2, ry = y - RH/2;
  const bg = ctx.createLinearGradient(rx,ry,rx,ry+RH);
  bg.addColorStop(0,'#0d1f3a'); bg.addColorStop(1,'#080f1e');
  ctx.fillStyle = bg; ctx.strokeStyle = color; ctx.lineWidth = 2;
  ...
  ctx.fillStyle = sg;
  ctx.beginPath(); ctx.roundRect(rx,ry,RW,3,[8,8,0,0]); ctx.fill();
  ctx.fillStyle = '#ddeeff'; ctx.font = 'bold 10px IBM Plex Mono'; ctx.textAlign = 'center';
  ctx.fillText(label, x, y - 4);
  ctx.fillStyle = '#4a6a90'; ctx.font = '8px IBM Plex Mono';
  sub.split('\n').forEach((l,i) => ctx.fillText(l, x, y + 7 + i*10));
}